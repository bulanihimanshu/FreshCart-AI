...
- [x] **API-04**: `GET /api/products?query=&limit=` — returns products matching the query string by name, up to the specified limit
- [x] **API-05**: `GET /api/products/{id}` — returns a single product's `{ product_id, name, aisle, department }`
