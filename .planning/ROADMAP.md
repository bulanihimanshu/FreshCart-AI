...
| 5 | Time-Aware LSTM | Add time-context embeddings (hour, DOW, days_gap), retrain, compare | ML-05 | 3 |
...
**Plans:**
1. `05_Time_Features.ipynb` (embedding layers) — add Embedding(24, 16) for hour, Embedding(7, 8) for DOW, Dense(8) for normalized days_gap; concatenate 32-dim time vector with 256-dim LSTM output → 288-dim → Dense softmax
